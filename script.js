...
const account3 = {
  owner: 'Denis Petrov',
  movements: [2000, 1400, 150, -890, 3010, -1100, 18500, 30],
  pin: 3333,
...
const formatMovementsDate = function (date, locale) {
  ...
  if (daysPassed <= 7) {
    const daysWord = getDaysWord(daysPassed);
    return `${daysPassed} ${daysWord} назад`;
  }
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const typeOfMov = mov > 0 ? 'внесено' : 'снятие';
...
const startLoginOutTimer = function () {
  ...
  return timer;
  //распечатать оставшееся время в UI

  //Когда время 0 сек, останвоить таймер и выйти из логина
};
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    //Создаём текущую дату после авторизации
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    //Сброс таймера если выполнить действия
    clearInterval(timer);
    timer = startLoginOutTimer();
  }
});
